...
QByteArray hexToBytes(const QString &hex, bool *ok){
    ...
    for (int i = 0; i < cleaned.length(); i += 2) {
        bool convOk = false;
        quint8 byte = static_cast<quint8>(cleaned.mid(i, 2).toUInt(&convOk, 16));
        if (!convOk) {
            if (ok) *ok = false;
            return {};
        }
...
QString bytesToHex(const QByteArray &bytes, char separator){
    QString result;
    result.reserve(bytes.length()*3);
    for (int i = 0; i < bytes.size(); ++i) {
        if (i > 0 && separator != '\0')
            result.append(separator);
        result.append(QString::asprintf("%02X",static_cast<unsigned char>(bytes.at(i))));
    }
    return result;
}
        ...
        if (inEscape) {
            ...
            case 'x': {
                ...
                quint8 byte = static_cast<quint8>(
                    text.mid(i + 1, 2).toUInt(&hexOk, 16));
                if (!hexOk) {
                    if (ok) *ok = false;
                    return {};
                }
...
static const quint16 crc16ccittTable[256] = {
...
quint16 crc16CCITT(const QByteArray &data){
    ...
    for (int i = 0; i < data.size(); ++i) {
        ...
        crc = static_cast<quint16>((crc << 8) ^ crc16ccittTable[index]);
    }
    return crc;
}

// CRC16-Modbus 查找表（多项式 0x8005）
static const quint16 crc16ModbusTable[256] = {
...
quint16 crc16Modbus(const QByteArray &data){
    ...
    for (int i = 0; i < data.size(); ++i) {
        ...
        crc = static_cast<quint16>((crc >> 8) ^ crc16ModbusTable[index]);
    }
    return crc;
}
...
QString checksumString(const QByteArray &data, ChecksumType type){
    ...
    case ChecksumType::Sum:
        return QString::asprintf("%02X", sumChecksum(data));
    case ChecksumType::CRC16_CCITT:
        return QString::asprintf("%04X", crc16CCITT(data));
    case ChecksumType::CRC16_Modbus:
        return QString::asprintf("%04X", crc16Modbus(data));
    default:
        return {};
    }
}
...
QByteArray encode(const QString &text, const QString &encoding){
    if (encoding == "UTF-8") {
        ...
        return text.toUtf8(); // 回退
    } else if (encoding == "Latin1") {
        return text.toLatin1();
    }
    return text.toUtf8(); // 默认
}
...
QString decode(const QByteArray &data, const QString &encoding){
    if (encoding == "UTF-8") {
        ...
        return QString::fromUtf8(data); // 回退
    } else if (encoding == "Latin1") {
        return QString::fromLatin1(data);
    }
    return QString::fromUtf8(data); // 默认
}
